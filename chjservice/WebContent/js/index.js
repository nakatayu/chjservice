...
// alert(geoip_latitude());
  //  alert(geoip_city());
	...
	alert("Cookies is Disabled,请打开 Cookies !");
	}


   $.ajax({
     type: "POST",
     url : context+'/common!findWether.action',
     dataType:'json',
     data:{url:"http://www.weather.com.cn/data/cityinfo/101021200.html"},
     success: function(msg,status){
    	 var wether =msg["weatherinfo"];
    	 $('#wetherinfo').html('<span style="font-size:24px;">'+wether["temp1"]+'</span>/<span style="font-size:20px;">'+wether["temp2"]+'</span><br/><span style="font-size:26px;">'+wether["weather"]+'</span>');
	     $('#wetherday').html(msg["date"]+" "+msg["week"]);
	     var info = wether["weather"];
	     if(info.indexOf('晴')!=-1){
	    	 if(info.indexOf('云')!=-1 ||info.indexOf('阴')!=-1)
	    		 $('#img_weather').attr("src",context+"/images/w_sannyyuun.png");
	    	 else
	    		 $('#img_weather').attr("src",context+"/images/w_sunny.png");
	     }
	     else if(info.indexOf('雨')!=-1){
	    	 if(info.indexOf('雷')!=-1){
	    		 if(info.indexOf('云')!=-1)
	    			 $('#img_weather').attr("src",context+"/images/w_leirainyun.png");
	    		 else
		    		 $('#img_weather').attr("src",context+"/images/w_leirain.png");
	    	 }
	    	 else if(info.indexOf('中')!=-1){
	    		 $('#img_weather').attr("src",context+"/images/w_middlerain.png");
	    	 }
	    	 else  if(info.indexOf('小')!=-1){
	    		 $('#img_weather').attr("src",context+"/images/w_littlerain.png");
	    	 }
	    	 else  if(info.indexOf('雪')!=-1){
	    		 $('#img_weather').attr("src",context+"/images/w_rainsnow.png");
	    	 } 
	    	 else 
	    		 $('#img_weather').attr("src",context+"/images/w_rain.png");
	     }
 	    else if(info.indexOf('雪')!=-1){
 	    	 if(info.indexOf('中')!=-1)
 	    		 $('#img_weather').attr("src",context+"/images/w_middlesnow.png");
 	    	else  if(info.indexOf('小')!=-1)
	    		 $('#img_weather').attr("src",context+"/images/w_littlesnow.png");
	    	else
	 	    	$('#img_weather').attr("src",context+"/images/w_snow.png");
	     }
 	    else if(info.indexOf('雾')!=-1){
 	    	$('#img_weather').attr("src",context+"/images/w_fog.png");
	     }
 	    else if(info.indexOf('云')!=-1){
 	    	 if(info.indexOf('多')!=-1)
 	    		 $('#img_weather').attr("src",context+"/images/w_duoyun.png");
 	    	 else  if(info.indexOf('风')!=-1)
 	    		 $('#img_weather').attr("src",context+"/images/w_yunfeng.png");
 	    	 else  if(info.indexOf('雷')!=-1)
 	    		 $('#img_weather').attr("src",context+"/images/w_leirainyun.png");
 	    	 else
 	    		 $('#img_weather').attr("src",context+"/images/w_yun.png");
	     }
 	    else if(info.indexOf('风')!=-1){
 	    	 if(info.indexOf('台')!=-1)
 	    		 $('#img_weather').attr("src",context+"/images/w_taifeng.png");
 	    	 else
 	    		$('#img_weather').attr("src",context+"/images/w_wind.png");
 	    }
 	    else	 if(info.indexOf('雷')!=-1){
 	    	$('#img_weather').attr("src",context+"/images/w_lei.png");
 	    }
...
Hongru.shutter = function(){
	...
	init.prototype = {
		init:function(options){ //options参数：id（必选）：图片列表父标签id；auto（可选）：自动运行时间；index（可选）：开始的运行的图片序号
			var wp = document.getElementById(options.id), // 获取图片列表父元素
			ul = H$$('ul',wp)[0], // 获取
			li = this.li = H$$('li',ul);
			...
			this.nav_wp.style.cssText = 'width:350px;background:#9a9a9a;filter:alpha(opacity=85);-moz-opacity:0.65;opacity:0.65;position:absolute;right:0;bottom:195;padding:8px 0;'; //为它设置样式
		...
		auto:function(){
			this.li.a = setInterval(new Function(this.anchor+'.move(1)'),30000); 
		},
